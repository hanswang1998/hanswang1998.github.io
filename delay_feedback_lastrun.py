...
# Run 'Before Experiment' code from model_pattern_begin_code
...
expName = 'delay_feedback'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\why\\Desktop\\动态决策测验psychopy\\delay_feedback_lastrun.py',
...
image_3 = visual.ImageStim(
    win=win,
    name='image_3', 
    image='指导语/延迟反馈指导语/延迟反馈指导语.png', mask=None, anchor='center',
...
intro3Components = [button, image_3, key_resp_9]
...
for thisTrial in trials:
    ...
    #timecoder=1200-t+timecoder
    valueA=f"任务A价值:{VAR1}"
    valueB=f"任务B价值:{VAR2}"
    valueC=f"任务C价值:{VAR3}"
    showscore_text.setText(f"累计积分：{TotalScoreCount}")
    showappendscore_text.setText(f"+{round(doubleThisTrailScore,2)}")
    count=0
    Totaltime=Totaltime-tasktrmainingtime
    total_trial_show=f"剩余轮次：{total_trial_show_count}"
    
    valueA_text.setText(valueA
)
    valueB_text.setText(valueB
)
    valueC_text.setText(valueC
)
    # keep track of which components have finished
    ...
    if tasknumber=="A":
        taskresult=round(30+70*stats.percentileofscore(resultA_range,pattern_correct_count )/100,2)
    elif tasknumber=="B":
        taskresult=round(30+70*stats.percentileofscore(resultB_range,n_back_correct_count )/100,2)
    elif  tasknumber=="C":
        taskresult=round(30+70*stats.percentileofscore(resultB_range,vs_correct_count )/100,2)
    
    tasknumberlist.append(tasknumber)
    taskscorelist.append(taskresult)
    
    ThisTrailScore=round(taskresult*get_VAR(tasknumber)/100,1)
    finalscorelist.append(ThisTrailScore)    
    ...
    if df_rocount%2 ==0 :
            taskchose_text=f"您前两轮选择执行的任务是：任务{tasknumberlist[df_rocount-2],tasknumberlist[df_rocount-1]}"
            taskscore_text=f"您前两轮任务的总评分是：{round(taskscorelist[df_rocount-2]+taskscorelist[df_rocount-1],2)}"
            finalscore_text=f"您所获得的积分是：{round(finalscorelist[df_rocount-2]+finalscorelist[df_rocount-1],2)}"
            TotalScoreCount += finalscorelist[df_rocount-2]+finalscorelist[df_rocount-1]
            TotalScoreCount = round(TotalScoreCount,1)
            doubleThisTrailScore=finalscorelist[df_rocount-2]+finalscorelist[df_rocount-1]
    elif df_rocount%2 !=0 :
        doubleThisTrailScore =0
    # keep track of which components have finished
