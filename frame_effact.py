...
expName = 'frame_effact'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\why\\Desktop\\动态决策测验psychopy\\frame_effact.py',
...
showappendscore_text = visual.TextStim(win=win, name='showappendscore_text',
    ...
    languageStyle='LTR',
    depth=-19.0);
showpl = visual.TextStim(win=win, name='showpl',
    text='',
    font='Open Sans',
...
two_back_num_correct_text = visual.TextStim(win=win, name='two_back_num_correct_text',
    text='',
    font='MicroSoft YaHei',
...
for thisTrial in trials:
    ...
    valueA_text.setText(f"任务A损失:{VAR1}")
    valueB_text.setText(f"任务B损失:{VAR2}")
    valueC_text.setText(f"任务C损失:{VAR3}")
    showscore_text.setText(f"累计积分：{round(500-TotalScoreCount,1)}")
    ...
    while continueRoutine:
        ...
        if showtime_text.status == STARTED:  # only update if drawing
            showtime_text.setText(showtime
, log=False)
        ...
        ###时间显示模块
        showtime,timecoder=getdowntime(1200)
        ...
        # *showpl* updates
        if showpl.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            showpl.frameNStart = frameN  # exact frame index
            showpl.tStart = t  # local t and not account for scr refresh
            showpl.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(showpl, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'showpl.started')
            showpl.setAutoDraw(True)
        if showpl.status == STARTED:  # only update if drawing
            showpl.setText(polygon1h, log=False)
    ...
    # set up handler to look after randomisation of conditions etc
    trials_3 = data.TrialHandler(nReps=15.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
    ...
    for thisTrial_3 in trials_3:
        ...
        nbackdowntime=30
        if two_back_count ==2:
            nbackdowntime=int(round(core.CountdownTimer(30).getTime()))
        two_back_num_text.setText(two_back_num)
        key_resp_3.keys = []
        key_resp_3.rt = []
        _key_resp_3_allKeys = []
        two_back_num_correct_text.setText(two_back_correct)
        # keep track of which components have finished
        n_backComponents = [two_back_num_text, key_resp_3, two_back_num_correct_text, two_back_num_showtime_text]
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            if two_back_num_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > two_back_num_text.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # *two_back_num_correct_text* updates
            if two_back_num_correct_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                two_back_num_correct_text.frameNStart = frameN  # exact frame index
                two_back_num_correct_text.tStart = t  # local t and not account for scr refresh
                two_back_num_correct_text.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(two_back_num_correct_text, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'two_back_num_correct_text.started')
                two_back_num_correct_text.setAutoDraw(True)
            if two_back_num_correct_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > two_back_num_correct_text.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    two_back_num_correct_text.tStop = t  # not accounting for scr refresh
                    two_back_num_correct_text.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'two_back_num_correct_text.stopped')
                    two_back_num_correct_text.setAutoDraw(False)
            ...
            if two_back_num_showtime_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > two_back_num_showtime_text.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        while continueRoutine and routineTimer.getTime() < 0.5:
            ...
            if two_back_feedback_text1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > two_back_feedback_text1.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if two_back_showtime_text2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > two_back_showtime_text2.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    # completed 15.0 repeats of 'trials_3'
    ...
    for thisTrial_2 in trials_2:
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            if VSimage.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > VSimage.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if taskctime_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > taskctime_text.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    taskresultA=100-random.randrange(25,35,1)
    taskresultB=100-random.randrange(15,25,1)
    taskresultC=100-random.randrange(5,15,1)
    taskchose_text=f"您选择执行的任务是：任务{tasknumber}"
    if tasknumber=="A":
        taskscore_text=f"您本轮任务的评分是：{taskresultA}"
        finalscore_text=f"您本轮的损失是：{round(taskresultA*VAR1/100,1)}"
        ThisTrailScore=round(taskresultA*VAR1/100,1)
        if VAR1<95 and VAR2>5 and VAR3 >5:
            VAR1 +=10
            VAR2 -=5
            VAR3 -=5
            checkmove ="A"
        ...
        finalscore_text=f"您本轮的损失是：{round(taskresultB*VAR2/100,1)}"
        ThisTrailScore=round(taskresultB*VAR2/100,1)
        if VAR2<95 and VAR1>5 and VAR3 >5:
            VAR1-=5
            VAR2+=10
            VAR3-=5
            checkmove = "B"
        ...
        finalscore_text=f"您本轮的损失是：{round(taskresultC*VAR3/100,1)}"
        ThisTrailScore=round(taskresultC*VAR3/100,1)
        if VAR3<95 and VAR2>5 and VAR1 >5:
            VAR1-=5
            VAR2-=5
            VAR3+=10
            checkmove = "C"
    ...
    # keep track of which components have finished
    feedbackComponents = [feedback_polygon, showresult1, taskscore_text1, finalscore_text1, feedback_button]
