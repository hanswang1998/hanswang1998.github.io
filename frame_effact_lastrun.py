...
Totaltime=600
tasktrmainingtime=0
total_tiral_show_count=16
total_tiral_show=f"剩余轮次：{total_tiral_show_count}"
###时间显示模块
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\why\\Desktop\\动态决策测验psychopy\\frame_effact_lastrun.py',
...
image = visual.ImageStim(
    win=win,
    name='image', 
    image='指导语/框架效应指导语/框架效应指导语1.png', mask=None, anchor='center',
...
image_3 = visual.ImageStim(
    win=win,
    name='image_3', 
    image='指导语/框架效应指导语/框架效应指导语2.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(1.92,1.08),
    color=[1.0000, 1.0000, 1.0000], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-1.0)
...
for thisTrial_5 in trials_5:
    ...
    Intro1Components = [key_resp_7, image]
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if showtime_text.status == STARTED:  # only update if drawing
            showtime_text.setText(total_tiral_show
, log=False)
    ...
    taskresultA=100-round(30+70*stats.percentileofscore(resultA_range,pattern_correct_count )/100,2)
    taskresultB=100-round(30+70*stats.percentileofscore(resultB_range,n_back_correct_count )/100,2)
    taskresultC=100-round(30+70*stats.percentileofscore(resultB_range,vs_correct_count )/100,2)
    taskchose_text=f"您选择执行的任务是：任务{tasknumber}"
    if tasknumber=="A":
        tasktrmainingtime=taskatime
        taskscore_text=f"您本轮任务的评分是：{round(100-taskresultA)}"
        ...
        taskscore_text=f"您本轮任务的评分是：{round(100-taskresultB)}"
        ...
        taskscore_text=f"您本轮任务的评分是：{round(100-taskresultC)}"
