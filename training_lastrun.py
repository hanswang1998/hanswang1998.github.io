...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\why\\Desktop\\动态决策测验psychopy\\training_lastrun.py',
...
image = visual.ImageStim(
    win=win,
    name='image', 
    image='指导语/测验总指导语/测验总指导语.png', mask=None, anchor='center',
...
image_2 = visual.ImageStim(
    win=win,
    name='image_2', 
    image='指导语/测验总指导语/训练阶段指导语.png', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(1.92, 1.08),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
...
for thisTrial_2 in trials_2:
    ...
    file_handle=open(f"预实验数据未放入/result{expInfo['participant']}.txt",mode='a')
...
for thisTrial_4 in trials_4:
    ...
    for thisTrial_3 in trials_3:
        ...
        while continueRoutine:
            ...
            if two_back_num_text.status == STARTED:  # only update if drawing
                two_back_num_text.setColor([1.0000, 1.0000, 1.0000], colorSpace='rgb', log=False)
...
for thisTrial_6 in trials_6:
    ...
    print(f"视觉搜索第{rowccount}轮：{vs_correct_count}\n")
    file_handle=open(f"预实验数据未放入\result{expInfo['participant']}.txt",mode='a')
